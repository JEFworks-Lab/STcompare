...
Additionally, for each pairwise kidney SpatialExperiment object in the list of 100 random simulations, the corrected p-value can be computed using `spatialCorrelationGeneExp` for each pair. To reduce runtime for the tutorial, we provide the output from running this function as `simRanPatternResults.RData`. 
...
The blue is the naive p-value for a given pair and the green is the permuted p-value for a given pair. 
Without p-value correction, we observe that 50% of these analytical p-values are less than 0.05, 
