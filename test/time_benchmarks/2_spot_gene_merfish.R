...
dir.create("outputs/merfish_logs", recursive = TRUE, showWarnings = FALSE)
dir.create("outputs/merfish_corr_output", recursive = TRUE, showWarnings = FALSE)

summary_file <- "outputs/merfish_benchmark_summary.csv"
...
run_benchmark <- function(resolution, run_num) {
...
for (res in resolutions) {
  for (run_num in seq_len(n_runs)) {
    row <- run_benchmark(resolution = res, run_num = run_num)
    all_results[[idx]] <- row
    idx <- idx + 1
