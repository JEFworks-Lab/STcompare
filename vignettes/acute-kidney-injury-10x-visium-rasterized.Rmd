...
After performing CPM normalization, when we plot the total expression of all genes in each pixel, the value of each pixel should be 1 million (1e6).
...
plot_gene_spatial <- function(gene, input, sharedPixels) {
  ...
  ctrl_df <- data.frame(
    ...
    color = SummarizedExperiment::assay(input[[1]])[gene, sharedPixels]
  )

  # AKI dataframe
  aki_df <- data.frame(
    SpatialExperiment::spatialCoords(input[[2]])[sharedPixels, ],
    color = SummarizedExperiment::assay(input[[2]])[gene, sharedPixels]
  )
...
gene1_plt <- plot_gene_spatial(gene1, input, sharedPixels)
gene2_plt <- plot_gene_spatial(gene2, input, sharedPixels)
...
start_time <- Sys.time()
ss <- STcompare::spatialSimilarity(input, foldChange = 1, assayName = "pixelval", t1 = NULL, t2 = NULL)
end_time <- Sys.time()
print(end_time-start_time)


# Figure 2I - fold change linear regression and spot classification 
gene1_lr <- STcompare::linearRegression(ss, gene=gene1, assayName = "pixelval") + ggplot2::theme_classic() + ggplot2::coord_fixed()
cbr1_lr <- STcompare::linearRegression(ss, gene=gene2, assayName = "pixelval") + ggplot2::theme_classic() + ggplot2::coord_fixed()

gene1_pc <- pixelClass(ss, gene=gene1, assayName = "pixelval")
gene2_pc <- pixelClass(ss, gene=gene2, assayName = "pixelval")

gene1_plt$control + gene1_plt$aki + gene1_corr + gene1_lr + gene1_pc
