...
plot_df <- data.frame(
  ...
  empirical_pval = cors_df$log_corrected
)
...
plt <- ggplot(plot_df_long, aes(x = type, y = neglog10_pval, color = type)) +
  ...
  labs(
    x = "P-value type",
    y = "-log10(p-value)",
    title = "BH-corrected Analytical p-value (pA) vs BH-corrected empirical p-value (pE)"
  ) +
  theme(
...
# percentage of signficant pairs based on the BH corrected naive p-value
naive_sig <- plot_df[plot_df$analytical_pval > -log10(0.05), ]
naive_sig_per <- dim(naive_sig)[1] / dim(plot_df)[1]
naive_sig_per
dim(naive_sig)[1]

empirical_sig <- plot_df[plot_df$empirical_pval > -log10(0.05), ]
empirical_per <- dim(empirical_sig)[1] / dim(plot_df)[1]
empirical_per
dim(empirical_sig)[1]
...
The median for the naive p-value is higher than the empirical one and there are more pairs that are above the significance line 
in the naive p-value plot than in the empirical p-value. 
We observe that ~43% of these analytical p-values are less than 0.05, 
while only ~4% of the empirical p-values are less than 0.05. 
