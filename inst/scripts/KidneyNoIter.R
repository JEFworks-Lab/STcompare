...
# TODO: there are duplicates in rast for some reason 

# x <- setdiff(rownames(moransI_ctrl), make.names(rownames(rast$AKI_ctrl)))
# length(x)

# y <- setdiff(make.names(rownames(rast$AKI_ctrl)), rownames(moransI_ctrl))
# length(y)

# # set up parameters for spatialCorrelationGeneExp 
# rownames(rast$AKI_ctrl)[duplicated(rownames(rast$AKI_ctrl))]
# sum(duplicated(rownames(rast$AKI_ctrl)))

# rownames(rast$AKI_aki)[duplicated(rownames(rast$AKI_aki))]
# sum(duplicated(rownames(rast$AKI_aki)))

# choosing the genes that are SVGs in both conditions 
genes_chosen <- svg_int

# TODO: the work around 
...
kidneyCorrelationNoIter <- spatialCorrelationGeneExp(
  input, 
  nPermutations = 100, 
  deltaX = deltaList, 
  deltaY = deltaList, 
...
save(kidneyCorrelationNoIter, file = file.path("inst", "extdata", "kidneyCorrelationNoIter.RData"))
