...
# Out of 32285 genes: 
# there are 1669 genes that are SVGs in the control dataset 
length(svg_ctrl)

# there are 2319 genes that are SVGs in the aki dataset  
length(svg_aki)

# there are 1044 genes that are SVGs in both datasets   
length(svg_int)

# there are 2944 genes that are SVGs in either datasets 
length(svg_uni)
```
...
setdiff(svg_test_2, svg_test1)
setdiff(svg_test1, svg_test_2)
...
cbr1_lr <- STcompare::linearRegression(ss, gene=gene2, assayName = "CPM") + ggplot2::theme_classic() + ggplot2::coord_fixed()
